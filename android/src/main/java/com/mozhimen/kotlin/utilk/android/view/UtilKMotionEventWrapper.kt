...
fun MotionEvent.motionEvent2str(): String =
    UtilKMotionEventWrapper.motionEvent2str(this)

object UtilKMotionEventWrapper {
    ...
    fun getDistance(motionEvent: MotionEvent): Float =
        if (motionEvent.pointerCount >= 2)
            UtilKMathPoint.getDistance(motionEvent.getX(0), motionEvent.getY(0), motionEvent.getX(1), motionEvent.getY(1))
        else 0f
    ...
    fun isTapInArea(event: MotionEvent, left: Float, top: Float, right: Float, bottom: Float): Boolean =
        UtilKMathArea.isInside_android(event.x, event.y, left, top, right, bottom)

    /**
     * 是否点击区域内
     */
    @JvmStatic
    fun isTapOutArea(event: MotionEvent, left: Float, top: Float, right: Float, bottom: Float): Boolean =
        UtilKMathArea.isOutside_android(event.x, event.y, left, top, right, bottom)

    /**
     * 是否点击区域内
     */
    @JvmStatic
    fun isTapInArea_raw(event: MotionEvent, left: Float, top: Float, right: Float, bottom: Float): Boolean =
        UtilKMathArea.isInside_android(event.rawX, event.rawY, left, top, right, bottom)

    /**
     * 是否点击区域内
     */
    @JvmStatic
    fun isTapOutArea_raw(event: MotionEvent, left: Float, top: Float, right: Float, bottom: Float): Boolean =
        UtilKMathArea.isOutside_android(event.rawX, event.rawY, left, top, right, bottom)
